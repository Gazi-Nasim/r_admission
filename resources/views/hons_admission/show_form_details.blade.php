    ...
    <div class="panel panel-default">
        <div class="panel-body">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <p class="text-right">
                        <a class="btn btn-danger btn-xs" href="{{ route('student.getDashboard') }}"><i class="fa fa-arrow-left"></i> Back</a>
                        <a class="btn btn-primary btn-xs" href="{{ route('hons_admission.downloadAdmissionForm',$subjectOption->id) }}"> <i class="fa fa-download"></i> Download Registration Form</a>
                        @if($subjectOption?->admission_completed ==1 && $subjectOption?->office_status!='-1')
                            <input type="hidden" name="_token" id="_token" value="{{csrf_token()}}">
                            <a class="btn btn-success btn-xs"
                               ic-target='#message'
                               ic-include="#_token"
                               ic-post-to='{{ route('hons_admission.apply', [$subjectOption->id]) }}'
                               role="button">
                        ...
                        <tr>
                            ...
                            <td>
                                @if($subjectOption->bill->payment_status == 0)
                                    Unpaid |
                                    <form style="margin-top: 10px; display: inline"  class="form-inline" method="post" action="{{ route('rocket.pay') }}">
                                        @csrf
                                        <input type="hidden" name="bill_id" id="bill_id" value="{{$subjectOption->bill->id}}">
                                        <button type="submit" class="btn btn-xs btn-primary"><i class="fa fa-money"></i>
                                            Pay Online
                                        </button>
                                    </form>

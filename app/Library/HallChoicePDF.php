...
class HallChoicePDF
{
    ...
    public static function makePDF($subjectOptionId)
    {
        ...
        $studentChoices = $subjectOption->hallChoices()->with('hall')
            ->orderBy('priority')
            ->get();
        ...
        if ($studentChoices) {
            ...
            $template_path = public_path().'/assets/pdf/hall_choice_template.pdf';
            ...
            $subject      = $subjectOption->admission_subject->name;
            ...
            $mpdf->WriteText(58, 98.4 + $offset, "$subject");
            $mpdf->WriteText(58, 103.2 + $offset, "$quota");
            $mpdf->WriteText(58, 108 + $offset, "$metir");
            $mpdf->WriteText(58, 113 + $offset, "$last_update");
            ...
            $html .= '<table align="center" width="90%" border="1" style="margin-top:3.55in; border-collapse: collapse">';
            $html .= ' <tr>';
            $html .= ' <th>Hall</th>';
            $html .= ' <th>Choice</th>';
            $html .= ' </tr>';
            ...
            foreach ($studentChoices as $choice) {
                $html .= ' <tr>';
                $html .= ' <td style="padding: 0 5px">'.$choice->hall->name.'</td>';
            ...
            $file_name = 'Hall-Choice-form_'.strtoupper($unit).$roll.".pdf";
